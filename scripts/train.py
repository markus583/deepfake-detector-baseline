...
parser.add_argument(
    "--weights", help="continue training or fine-tune", type=str, default=""
)
parser.add_argument("--encoder", help="train on only one encoder", type=str, default="")
parser.add_argument("--data_setup", type=str, default="")
...
loader = AudioLoader(POS_DB_PATH, NEG_DB_PATH, global_conf)
...
if "patch_size_t" in global_conf:
    ...
    it_val = loader.create_tf_iterator(
        "validation", augmenter=augmenter, encoder=ENCODER
    )
...
if not ENCODER:
    model.m.compile(
        ...
        loss_weights={"deepfake": 1.0, "encoder": 0.0},
        metrics={
...
if not ENCODER:
    save_conf(
        global_conf, loader.params, model.params, augmenter.params, name=args.config
    )

saver = tf.keras.callbacks.ModelCheckpoint(
    "{}/{}{}".format(WEIGHTS_PATH, global_conf["name"], ENCODER),
    monitor="val_loss",
    verbose=1,
)
early_stop = tf.keras.callbacks.EarlyStopping(monitor="val_loss", patience=20)
reducer = tf.keras.callbacks.ReduceLROnPlateau(
    monitor="val_loss", patience=10, factor=0.1, mode="auto", min_lr=1e-5
)

model.m.fit(
    it_train,
    epochs=200,
    steps_per_epoch=10 * global_conf["repeat"],
    verbose=1,
    validation_data=it_val,
    validation_steps=10 * global_conf["repeat"],
