...
class SimpleCNN:
    def __init__(self, input_shape, params={}, detect_encoder=False):
        ...
        for h, s, k in zip(
            self.params["hidden"], self.params["strides"], self.params["kernel"]
        ):
        ...
        if self.params["head_dense"] > 0:
            self.fc = Dense(
                self.params["head_dense"], activation=self.params["dense_activation"]
            )
            Y = self.fc(Y)
            Y = Dropout(0.2)(Y)
        ...
        if not detect_encoder:
            ...
            self.fc_encoder = Dense(
                detect_encoder, activation="softmax", name="encoder"
            )
            Y1 = self.fc_out(
                Y,
            )
...
class SimpleSpectrogramCNN:
    def __init__(
        self, input_shape, params={}, detect_encoder=False, get_embedder=False
    ):
        self.name = "specnn"
